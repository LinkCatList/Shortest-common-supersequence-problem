...
public:
  ...
  string buildDnaString(){
    ...
    unordered_set<string>NonDelete;
    for(int i = 0; i<this->DnaSize; i++){
      NonDelete.insert(this->Dna[i]);
    }
    while(NonDelete.size() != 1){
      this->Dna.clear();
      for(auto Word:NonDelete){
        this->Dna.push_back(Word);
      }
      ...
      for(auto FirstString:NonDelete){
        for(auto SecondString:NonDelete){
          ...
          if(this->getMaxCommonSuffix[{FirstString, SecondString}]>=MaximumOverlap){
            MaximumOverlap = this->getMaxCommonSuffix[{FirstString, SecondString}];
